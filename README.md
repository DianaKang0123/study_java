...
- JDK : Java Development Kit
  ...
  - 가장 큰 프로그램, 디버깅 등의 개발 툴을 제공
- JRE : Java Class Library
  - Math, Swing 등의 자바 내장 라이브러리 제공
  - JDK 에 포함 되어있음
- JVM : Java Virtual Machine
  - 자바 가상 머신, 자바를 각 os 에 맞게 컴파일

### 빌드, 실행
- 빌드(build) :
  - 소스 코드 파일을 컴퓨터에서 실행할 수 있는 독립 SW 가공물로 변환 시키는 과정
  - 독립 소프트웨어 가공물 = Artifact
  - 소스코드 컴파일 - 테스트 코드 컴파일 - 실행 - 테스트 코드 리포트 작성 - 추가설정 작업 - 패키징 - 최종 결과물 (artifact)
  - 테스트 코드 : 내가작성한 코드를 자동 테스트 해주는 코드를 작성한 것
- 실행(run):
  - 작성한 코드 혹은 테스트 코드를 컴파일을 거쳐 작동시키는 것
  - artifact 가 나올 수 도, 나오지 않을 수 도 있다
    - 인터프리터 언어(파이썬 등)은 컴파일이 필요 없다.
...
- 빌드 툴:
  ...
  - Apache ant
    - 설정을 위해 xml 사용
    - 간단 사용 편리
    - 복잡한 처리 시 빌드 스크립트가 복잡해서 관리의 어려움
    - 외부 라이브러리 관리 구조가 없음
  - Maven
    - 설정을 위해 xml을 사용
    - 외부 라이브러리 관리 가능
    - ant의 문제점인 빌드 스크립트 문제 해결
    - 특정 경우 xml이 복잡해짐
    - xml 자체의 한계가 존재
  - Gradle
...
## 형 변환
- Casting : 변수의 타입을 변환 시키는 것.
  - 자동 형변환 : 크기가 작은 타입을 큰 타입으로 변환
    - int -> long -> double
    - 기존 변수명 = (새로운 타입) 변수명; 으로 사용
    - 이 경우 문제가 발생하지 않기에 자동 형 변환이 가능함
  - 명시적 형변환 : 크기가 큰 타입을 작은 타입으로 변환 시키는 것
    - double -> int
  - 오버플로우
    - 크기가 작은 타입에서 큰 타입으로 변환 하는경우
    - 최대와 최솟값을 순환하듯 더 많은 값 만큼 최솟값에서 마이너스 되는 현상
    - 기존 숫자와 전혀 다른 숫자가 표현되는 현상
